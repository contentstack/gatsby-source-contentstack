...
export class ContentstackGatsby {
  ...
  async get(data) {
    ...
    if (live_preview?.hash && live_preview.hash !== 'init') {
      ...
      if(!receivedData.cslp__meta) {
        throw new Error("Entry data must contain cslp__meta for live preview")
      }
      ...
      const entryCslpMeta = receivedData.cslp__meta[entryUid];
      const paths = entryCslpMeta && entryCslpMeta.referencePaths ? entryCslpMeta.referencePaths : [];
      const jsonRtePaths = entryCslpMeta && entryCslpMeta.jsonRtePaths ? entryCslpMeta.jsonRtePaths : [];
